...
| ~~Load: `serde_json` parses every event's `ts_qpc` as a full `u64` through the generic visitor; a hand-rolled line parser for the fixed `{"ts_qpc":..,"dx":..,"dy":..}` shape would roughly halve the ~1.3 s load~~ | −76% on the 10 M-cell bench, 3.1 s → 0.5 s on a 1156 MB recording | **Superseded 2026-09-21:** parallelism beat a faster scalar parser and keeps serde. Recordings over 8 MB are split at line boundaries and parsed on every core (`load::load_parallel`, `docs/PERFORMANCE-2026-09-20.md` A1). |
...
| ~~`hypot` → `sqrt(x*x + y*y)` in the moving-cell loop (22 M calls)~~ | ~250 ms — the 2026-09-20 profile priced MSVC's `_hypot` at 8.5% of all analyzer CPU, 5× this estimate | **Implemented 2026-09-21:** `stats::mag` at all 17 sites. No parity test needed loosening: the dense reference computes magnitudes the same way, and a line-by-line diff of full reports over 19 M events found no differing value at all. |
